...
> **Design decision D-06-3 (page-truncation policy).** Read `getIndexedState`. If it is `INDEX_STATE_PARTIAL`, read `Zotero.Fulltext.getPages(itemID)` (verified, `fulltext.js:2799`; returns `{ indexedPages, total }`). If `total > indexedPages` **and** `total <= FULLTEXT_HARD_PAGE_CAP` (default 60), bypass the cache and call `Zotero.PDFWorker.getFullText(attachment.id, null)` to get all pages. Otherwise accept the truncated cache and set `sourceTruncated: true` on the summary, which the report surfaces in §9.2 Scope & Method. Never silently summarize a truncated document as if it were complete.
...
> **Unverified:** the precise serialization of `getStructuredDocumentText` (it is packed via `packStructuredDocumentText` from a `structured-document-text` sub-package) was not decoded during this research. It plausibly carries per-character geometry usable for column/heading detection, which would improve §4.3 IMRaD segmentation considerably.
>
