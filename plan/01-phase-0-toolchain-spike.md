...
|---|---|
| **ID** | `P0-T12` |
| **State** | `DONE` — completed 2026-09-10; vitest 5.0.0, 20 specs, three corpus defects fixed |
| **Depends on** | `P0-T05` |
| **Blocks** | `P0-T14` |
| **Retires** | none |
| **Implements** | none |
...
**Findings, 2026-09-10.** Resolved unknown: the corpus names Vitest but pins no version —
installed **`vitest@5.0.0`** with **`@vitest/coverage-v8@5.0.0`** on Node v22.23.0. 20 specs
pass, `src/model/ids.ts` is at 100% (11/11 statements, 6/6 branches, 1/1 functions, read from
`coverage/coverage-final.json`). It does **not** appear in the terminal coverage table: Vitest 5
turns on `coverage.skipFull` automatically when it detects an agent environment, so fully
covered files are hidden. Do not read that absence as "uncovered".

Three corpus defects were found by running the code rather than reading it, and all three are
now fixed in place:

1. **`docs/13` §2.1's `Zotero` fake did not compile.** `FakeItem` declared
   `public id = FakeItem.nextId++` above `static nextId = 1`, which is fine at runtime but is
   `TS2729` under this project's `tsconfig.json`. The two declarations are now swapped in
   §2.1, with a comment saying why, so the next person to copy the block verbatim does not
   hit it.
2. **`docs/07` §2.2's tree had no `test/setup/`**, although both this card's `Files` list and
   `docs/13` §2.1 name `test/setup/zotero-global.ts`. §2.2 is the authority for the tree, so
   §2.2 gained the directory.
3. **`npm test` failed on a green tree.** `test:contract` points at an empty
   `test/contract/`, and Vitest 5 exits **1** on "No test files found" — so the card's intended
   "known no-op" was in fact a hard failure that `P0-T14` would have wired straight into CI.
   `--passWithNoTests` added, in `package.json` and in `docs/13` §1.6, with a note to remove it
   once the directory has specs.

**No ESLint exemption was needed for the fake, contrary to the brief this card was executed
under.** `P0-T04`'s `no-restricted-globals` rule reports only references that resolve to the
*global* scope; the setup file binds `const Zotero = { … }` at module scope, so every reference
inside it resolves locally and the rule never fires. That is a real difference from
`scripts/probe-types.ts`, which names the true global.

**One behaviour of `docs/02` §11.1 worth a decision at `P1-T01`.** Its trailing-punctuation
strip `[.,;)\]]+$` has no leading counterpart, so `"(10.1056/nejmoa2300709);"` normalizes to
`null`: the closing paren comes off, the opening one does not, and the `^10\.` anchor then
fails. DOIs extracted from parenthesized prose will be dropped. This is §11.1's specified
behaviour and was implemented as specified, not "fixed"; the spec asserts the un-parenthesized
form instead.
...
|---|---|
| **ID** | `P0-T31` |
| **State** | `DONE` — completed 2026-09-10; criterion [1] and its grep were corrected, see Findings |
| **Depends on** | `P0-T07` |
| **Blocks** | `P0-T10` |
| **Retires** | none |
...
**Done when.**
- [ ] `src/zotero/registrations.ts` is the only file under `src/` that **calls** any of the
      four APIs. A doc comment that merely names one does not count; see `Notes`.
...
npm run lint:check && npm run typecheck \
  && grep -rlnE 'Zotero\.(MenuManager\.registerMenu|Notifier\.registerObserver|PreferencePanes\.register|Prefs\.registerObserver)\(' src/ \
...
**Findings, 2026-09-10.** Implemented as four `no-restricted-syntax` selectors, not
`no-restricted-properties`. **This card's claim about `no-restricted-properties` was tested
rather than trusted, and held:** `{ object: "MenuManager", property: "registerMenu" }` never
fires, because the outer `MemberExpression`'s `object` is itself a `MemberExpression` and so
`object.name` is `undefined`. The one variant that does fire —
`{ object: "Zotero", property: "MenuManager" }` — bans the whole manager including
`unregisterMenu`, which is backwards.

Each selector carries `:not(AssignmentExpression > MemberExpression.left)`, which is what
separates *calling* a registration API from *defining* one. Verified empirically both ways: it
still fires on `Zotero.Notifier.registerObserver(...)` **and** on the aliasing escape
`const alias = Zotero.Notifier.registerObserver`, but not on
`Zotero.Notifier.registerObserver = fn`. So no `test/**` exemption was needed for the Vitest
fake. Exemptions are two named files — `src/zotero/registrations.ts` and, because it calls all
four, `P0-T06`'s type-only `scripts/probe-types.ts` — never a directory.

Observed failure on a deliberate violation placed *inside* `src/zotero/`, proving the directory
is not exempt:

```
5:18  error  FR-56: use menuRegistration() from src/zotero/registrations.ts; a bare
             Zotero.MenuManager.registerMenu is a registration nothing will undo
             no-restricted-syntax
```

**This card's own `Verify with` grep was defective and has been corrected.** As first written
it matched `registerMenu|registerObserver|PreferencePanes\.register` anywhere, including the
doc-comment prose in `src/bootstrap/container.ts` and `src/bootstrap/registerUI.ts` that
*names* those APIs while explaining why nothing may call them — so it reported `LEAK` on a
correct tree. The criterion's intent was always call sites; the grep was a bad expression of
it, and both the grep and the criterion wording now say so.

Three things recorded rather than fixed:

- **Computed access slips the rule.** `Zotero.MenuManager["registerMenu"](…)` has
  `property.name === undefined`. Closing it would double the rule's size for a hole nobody
  reaches by accident.
- **The rule now also covers `typings/*.d.ts`.** `@zotero-plugin/eslint-config` switches
  `no-restricted-syntax` **off** for declaration files, and project overrides append after it,
  so this block re-enables it there. Harmless — a `.d.ts` expresses the same thing as a
  `TSQualifiedName`, never a `MemberExpression` — but it widens the shared config's intent.
- **`registrations.ts` imports `src/bootstrap/container.ts`, and nothing forbids it.**
  `docs/07` §2.3's layering arrow does not place `bootstrap/` anywhere at all, so an adapter
  importing the composition root is legal *by omission rather than by decision*. It is
  necessary here (`registration()` is a value, not just a type), but the corpus should say
  where `bootstrap/` sits before precedent decides for it.

**`Zotero.MenuManager.registerMenu` can return `false`, and nothing had handled that.**
